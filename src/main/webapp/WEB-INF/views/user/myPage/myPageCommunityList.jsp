...
<%@ include file="/WEB-INF/views/common/common.jspf" %>
		...
		<script type="text/javascript">
			$(function(){
				$(".content_wrap .page-header h1").html("자유 게시판");
			...
			function goPage(){
				...
				$("#f_search").attr({
					"method":"get",
					"action":"/member/myPageCommunityList"
				});
				$("#f_search").submit();
			}
		</script>
		...
		<div class="contentContainer container">
			...
			<div class="searchbox" style="margin-top: 0;">
				<form id="f_search" name="f_search">
					<input type="hidden" name="c_category" value="C" /> <input
						type="hidden" name="pageNum" value="${pageMaker.cvo.pageNum}">
					<input type="hidden" name="amount" value="${pageMaker.cvo.amount}">
					<input type="hidden" id="search" name="search" value="all" />
					<div class="keywordbox">
						<span class="keyword"> <input type="checkbox" name="key"
							id="all" class="m0" value="all" checked /> <label for="all">통합검색</label>
							<input type="checkbox" name="key" id="u_nick" value="u_nick"
							class="m0" /> <label for="u_nick">작성자</label> <input
							type="checkbox" name="key" id="c_title" value="c_title"
							class="m0" /> <label for="c_title">제목</label>
						</span>
					</div>
					<div class="searchbox_right block">
						<span> <input type="text" class="w280 pl10" name="keyword"
							id="keyword" value="검색어를 입력하세요" maxlength="30" />
						</span> <a href="#" id="searchData" class="btn_gray fr"><span
							class="icon"></span>검 색</a>
					</div>
				</form>
			</div>
			<%-- ===================== 검색 기능 종료 ===================== --%>
			...
			<div class="community_table mt30">
				<table>
					...
					<tbody id="list">
						<c:choose>
							<c:when test="${ not empty myPageCommunityList }">
								<c:forEach var="myPageCommunityList"
									items="${ myPageCommunityList }" varStatus="status">
									<tr class="text-center" data-num="${myPageCommunityList.c_no}">
										<td>${ myPageCommunityList.c_no }</td>
										<td class="goDetail tl">${ myPageCommunityList.c_title }</td>
										...
										<td>${ myPageCommunityList.c_created_at }</td>
										<td>${ myPageCommunityList.c_cnt }</td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
