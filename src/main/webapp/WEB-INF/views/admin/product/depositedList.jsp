	...
	<script>
		var btn = 0;
			...
			function goPage(){
				...
				$("#f_search").attr({
					"method":"get",
					"action":"/admin/product/depositedList"
				});
				$("#f_search").submit();
			}
			...
			function proValue(){
				...
				if(valueArr.length == 0){
					...
					$.each(valueArr, function(index, item){
						...
						$.ajax({
							url : "/admin/product/depositDelete",
							type : 'post',
							data : $("#depForm").serialize(),
							dataType :'text',
							success : function(data){
								location.href="/admin/product/depositedList"
	...
	<c:if test='${not empty adminLogin }'>
		<div class="contentContainer">
			...
			<div class="contentBtn text-right">
				<input type="button" value="입금취소" id="upBtn" class="btn btn-default">
			</div>
			<div id="div">
				<div class="boardList" class="table-height">
					<table summary="게시판 리스트" class="table table-striped">
						...
						<tbody class="list" class="table-striped">
							...
							<c:choose>
								<c:when test="${not empty depositedList }">
									<c:forEach var="deposit" items="${depositedList}" varStatus="status">
