...
<html>
	...
	$(function(){	
			$(".dashboard-wrapper .page-header h1").html("이유식게시판 관리");
		...
		function goPage(){
			...
			$("#f_search").attr({
				"method":"get",
				"action":"/admin/admCommunityList?c_category=B"
			});
			$("#f_search").submit();
		}
	</script>
...
<%-- =================== 리스트 시작  ================= --%>
		...
		<table summary="이유식 게시판 리스트" class="table table-striped m-t-15" >
			<thead>
				<tr>
							...
							<tr class="text-center" data-num="${community.c_no}">
								...
								<td>
									<c:if test="${not empty community.c_thumb}">
										<img src="/uploadStorage/communityB/thumbnail/${community.c_thumb}"/>
									</c:if>
